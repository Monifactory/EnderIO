...
public class ConduitSavedData extends SavedData {
    ...
    private ConduitSavedData(Level level, CompoundTag nbt) {
        ...
        for (Tag tag : graphsTag) {
            ...
            if (ConduitRegistries.getRegistry().containsKey(type)) {
                ConduitType<?> value = Objects.requireNonNull(ConduitRegistries.getRegistry().getValue(type));
    ...
    public CompoundTag save(CompoundTag nbt) {
        ...
        for (var entry : networks.entrySet()) {
            ...
            typedGraphTag.putString(KEY_TYPE, ConduitRegistries.getRegistry().getKey(type).toString());
    ...
    private void tick(ServerLevel serverLevel) {
        ...
        for (var entry : networks.entrySet()) {
            for (Graph<Mergeable.Dummy> graph : entry.getValue()) {
                if (serverLevel.getGameTime() % entry.getKey().getTicker().getTickRate() == ConduitRegistries.getRegistry().getID(entry.getKey()) % entry
                    .getKey()
                    .getTicker()
                    .getTickRate()) {
