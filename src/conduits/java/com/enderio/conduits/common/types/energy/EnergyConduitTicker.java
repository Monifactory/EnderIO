            ...
            for (NodeIdentifier<?> otherNode : loadedNodes) {
               for (Direction dir: Direction.values()) {
                   if (otherNode.getIOState(dir).map(NodeIdentifier.IOState::isInsert).orElse(false)) {
                       var be = level.getBlockEntity(otherNode.getPos().relative(dir));
                       if (be == null) {
                           continue;
                       }
                       var capability = be.getCapability(ForgeCapabilities.ENERGY, dir.getOpposite()).resolve();
                       if (capability.isPresent()) {
                           var insert = capability.get();
